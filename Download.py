...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📚 نصيحة اليوم", callback_data="maincat_نصيحة")]
    ]
...
async def send_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg_text:
        await update.message.reply_text("❌ اكتب الرسالة بعد الأمر: `/send نص الرسالة`", parse_mode="Markdown")
        return
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_panel" and user_id == ADMIN_ID:
        ...
        await query.edit_message_text("📢 أرسل الآن: `/send` ثم نص رسالتك.", parse_mode="Markdown")
        ...
        # إذا كانت القصة جزء واحد فقط، افتحها مباشرة
        if len(parts) == 1:
            keyboard = [[InlineKeyboardButton("🔙 عودة للقسم", callback_data=f"maincat_{cat}")]]
            # إذا كان الجزء الواحد طويلاً وفيه صفحات (===)
            if len(parts[0]) > 1:
            ...
            # إذا كانت القصة فيها أجزاء متعددة (NEXT_PART)
